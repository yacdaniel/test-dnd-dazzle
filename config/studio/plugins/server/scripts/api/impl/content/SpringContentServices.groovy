...
class SpringContentServices {
	...
	def writeContentAsset(site, path, fileName, content, isImage, allowedWidth, allowedHeight, allowLessSize, draft, unlock, systemAsset) {
		...
		def result = springBackendService.writeContentAsset(site, path, fileName, content, isImage, allowedWidth, allowedHeight, allowLessSize, draft, unlock, systemAsset);

		if(path.contains("/static-assets/images/dam")) {
			def url = "http://localhost:8080/api/dam/rendition.json?crafterSite=" + site + "&if=" + path + "/" + fileName + "&presets=thumb"
			def ytout = new URL(url).text
		}
		else if(path.contains("/static-assets/documents")) {
			def thread = Thread.start {
				def url = "http://localhost:8080/api/dam/doc-to-pdf.json?crafterSite=" + site + "&if=" + path + "/" + fileName
				def ytout = new URL(url).text
			}
		}
