 ...
 */
           ...
           overlayContent: function(sType, args, tree) {
            ...
            var getContent = function(site, path, edit, callback) { 
                ...
                var baseContentPath = path.replace("/es/", "/en/");
            ...
            lookupLayeredSiteContent: function(site, path, level, action, callback) {
                var combineResultsFn = function(baseTreeData, currentTreeData) {
                    ...
                    for(var key in indexByPath) {
                        ...
                        children[k].path = children[k].path.replace("/en", "/es");
                        children[k].browserUri = children[k].browserUri.replace("/en", "/es");
                        children[k].uri = children[k].uri.replace("/en", "/es");
                ...
                // NEED TO USE CONFIG ^
                var newPath = path.replace("/es", "/en")
...
/* ============================================= */
            ...
            initialize: function(config) {
                if (config.name == "layered-root-folder") {
                    ...
                    instance.label = config.params["label"];
            ...
            addContentTreeRootFolder: function(instance) {
                ...
                YDom.addClass(parentFolderEl, " fa fa-folder");
                parentFolderEl.style.fontStyle = "14px";
                parentFolderLinkEl.style.fontFamily = "sans-serif";

                parentFolderEl.style.lineHeight =  "15px";
                parentFolderEl.style.display = "inherit";
                parentFolderLinkEl.style.display = "inline";
                parentFolderLinkEl.style.margin = "5px";
            ...
            drawTreeItem: function(treeNodeTO, root, instance) {
                ...
                if (treeNodeTO.container == true || treeNodeTO.name != 'index.xml') {
                    ...
                    if (!treeNodeTO.style.match(/\bfolder\b/)) {
                        ...
                        YDom.addClass(icon, "fa studio-fa-stack-2x fa-file");
                        icon.style.color = "#8fc6fd";
                    ...
                    nodeSpan.style.fontFamily = "sans-serif";
                    YDom.removeClass(icon, "edited");
                    YDom.removeClass(icon, "fa-pencil");
                    var x = nodeSpan.children[0];
                    if(x && x.children[0]) {
                      x = x.children[0].children[1];  
                      YDom.removeClass(x, "edited");
                      YDom.removeClass(x, "fa-pencil");
                      x.style.color = "red";
		...
		return false;
    			                        ...
    			                        success: function(collection) {
                                            ...
                                            if ((collection.count > 0 && isContainer) && collection.item[0].uri.replace(/\/\//g,"/") != oCurrentTextNode.data.uri) {
                                                if(LSelf.myTree.getNodeByProperty("uri", collection.item[0].uri.replace(/\/\//g,"/"))){
                                                    if(LSelf.myTree.getNodeByProperty("uri", collection.item[0].uri.replace(/\/\//g,"/")).parent.contentElId != oCurrentTextNode.contentElId){
            ...
            onTriggerContextMenu: function(tree, p_aArgs) {
                ...
                var menuItems = {
					...
					newContentOption: { text: CMgs.format(siteDropdownLangBundle, "newContent"), onclick: { fn: LSelf.createContent } },
